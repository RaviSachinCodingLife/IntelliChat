...
export default function ChatBox() {
        ...
        (async () => {
            try {
                ...
                if (contentType && contentType.includes("application/json")) {
                    ...
                    if (data.messages) {
                        dispatch(setMessages(data.messages.map((m: any) => ({
                            sender: m.senderRole,
                            text: m.text,
    ...
    useEffect(() => {
        ...
        const ws = new WebSocket(
            `ws://localhost:5000?token=${encodeURIComponent(
                token
        ...
        ws.onmessage = (e) => {
            ...
            if (msg.type === "message") {
                if (msg.aiReply) {
                    ...
                    dispatch(addMessage({ sender: "ai", text: msg.aiReply, createdAt: new Date().toString() }));
                }
    ...
    function send(text: string) {
        ...
        dispatch(addMessage({ sender: "customer", text, createdAt: new Date().toString() }));
        dispatch(removeLoader());
